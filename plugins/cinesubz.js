...
const PROVIDER = {
    ...
    async search(query) {
        ...
        return findMovieLinks($, finalUrl, 'cinesubz.co');
    },
    ...
    async getDetails(movieUrl) {
        ...
        const title = extractTitle($, 'CineSubz Movie');
        const links = [];

        /*
         * CineSubz currently exposes its movie downloads
         * under a Links section and labels them with
         * quality + size (for example 480p / 720p / 1080p).
         */
        $('a[href]').each((_, el) => {
            const href = absUrl($(el).attr('href'), finalUrl);
            if (!href) return;
            ...
            if (!/(?:download|direct|telegram|480p|720p|1080p|2160p|4k)/i.test(combined)) {
                return;
            }
            ...
            if (
                !/\/(?:zt-links|links|download|dl|movies)\//.test(path) &&
...
export default {
    ...
    alias: ['csubz', 'cine'],
    category: 'download',
    desc: 'Search CineSubz movies and show available download qualities',
